  ...
  private void configureEclipse(final Project project, final SourceSet mainSourceSet, final SourceSet testSourceSet) {
    ...
    project.afterEvaluate(new Action<Project>() {
      ...
      public void execute(final Project project) {
        eclipseModel.classpath(new Action<EclipseClasspath>() {
          @Override
          public void execute(EclipseClasspath eclipseClasspath) {
            eclipseClasspath.getPlusConfigurations().addAll(Arrays.asList(
                    project.getConfigurations().getByName(new DslObject(mainSourceSet).getConvention().getPlugin(AptPlugin.AptSourceSetConvention.class).getCompileOnlyConfigurationName()),
                    project.getConfigurations().getByName(new DslObject(testSourceSet).getConvention().getPlugin(AptPlugin.AptSourceSetConvention.class).getCompileOnlyConfigurationName())
            ));
          }
        });
      }
    ...
    if (project.getTasks().findByName("eclipseJdtApt") == null) {
      GenerateEclipseJdtApt task = project.getTasks().create("eclipseJdtApt", GenerateEclipseJdtApt.class, new Action<GenerateEclipseJdtApt>() {
        ...
        public void execute(GenerateEclipseJdtApt generateEclipseJdtApt) {
          ...
          eclipseModel.getJdt().getFile().withProperties(new Action<Properties>() {
            @Override
    ...
    if (project.getTasks().findByName("eclipseFactorypath") == null) {
      GenerateEclipseFactorypath task = project.getTasks().create("eclipseFactorypath", GenerateEclipseFactorypath.class, new Action<GenerateEclipseFactorypath>() {
        ...
        public void execute(GenerateEclipseFactorypath generateEclipseFactorypath) {
          ...
          factorypath.setPlusConfigurations(new ArrayList<>(Arrays.asList(
                  project.getConfigurations().getByName(new DslObject(mainSourceSet).getConvention().getPlugin(AptPlugin.AptSourceSetConvention.class).getAptConfigurationName()),
                  project.getConfigurations().getByName(new DslObject(testSourceSet).getConvention().getPlugin(AptPlugin.AptSourceSetConvention.class).getAptConfigurationName())
          )));
        }
      });
      project.getTasks().getByName("eclipse").dependsOn(task);
